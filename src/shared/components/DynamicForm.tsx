...
export function DynamicForm({
	...
	const totalField = useMemo(
		() =>
			template.fields.find(
				(field) =>
					field.name.toLowerCase() === "total" ||
					field.label.toLowerCase() === "total",
			),
		[template.fields],
	);
	...
	const modalityFieldKey = useMemo(() => {
		const modalityField = template.fields.find(
			(f) =>
				f.type === "select" &&
				(f.name === "modalidad" || f.label.toLowerCase().includes("modalidad")),
		);
	...
	const renderFieldInput = (fieldDef: FormFieldDef, fieldApi: any) => {
		const isTotal =
			fieldDef.name.toLowerCase() === "total" ||
			fieldDef.label.toLowerCase() === "total";
			...
			<Input
				...
				onChange={(e) => {
					...
					fieldApi.handleChange(
						fieldDef.type === "number" ? (val ? parseInt(val, 10) : "") : val,
					);
				}}
				...
				{template.fields.map((fieldDef, index) => (
					<form.Field
						...
						children={(fieldApi) => (
							...
							>
								<Label
									htmlFor={`${fieldDef.id}@${fieldDef.name}`}
							...
							>
								{isSubmitting ? (
									<span className="flex items-center gap-2">
										<Loader2 className="animate-spin h-5 w-5" /> Guardando
										reporte...
									</span>
								) : (
									<>
