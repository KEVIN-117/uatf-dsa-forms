...
export function SuccessCard({
	isLoading = false,
	loadingText = "Cargando resumen de envíos...",
	variant = "default",
}: SuccessCardProps) {
	return (
		<Card className="w-full max-w-md mx-auto text-center glass-card shadow-lg border-border/40 overflow-hidden relative animate-fade-up">
			...
			<CardContent className="relative">
				{isLoading ? (
					...
					<p className="text-muted-foreground font-body leading-relaxed">
						Tu reporte ha sido enviado correctamente. Agradecemos tu
						participación y compromiso con los procesos académicos de la UATF.
					</p>
				)}
			</CardContent>
			...
			{!isLoading && variant === "default" && (
				<CardFooter className="relative flex justify-center pb-8">
					<Button
						asChild
						className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold px-8 hover-lift"
					>
