...
import {
	...
	useUpdateUser,
	useUsers,
} from "#/features/reference-data/useUsers";
...
export function DirectorsCrud() {
	...
	const openEditSheet = (item: User) => {
		setEditingItem(item);
		setForm({ ...item, role: "director" as Role });
		setSheetOpen(true);
	};
	...
	const handleDelete = async (item: User) => {
		...
		if (!confirm(`¿Eliminar director ${item.name} ${item.paternalSurname}?`))
			return;
	...
	const columns: ColumnDef<User>[] = [
		{ accessorKey: "ci", header: "CI" },
		{
			...
			header: "Nombre",
			cell: ({ row }) =>
				`${row.original.name} ${row.original.paternalSurname} ${row.original.maternalSurname}`,
		},
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				return count > 0 ? (
					<Badge
						variant="outline"
						className="text-emerald-600 border-emerald-500/40 bg-emerald-500/10 dark:text-emerald-400"
					>
						Enviado
					</Badge>
				) : (
					<Badge
						variant="outline"
						className="text-amber-600 border-amber-500/40 bg-amber-500/10 dark:text-amber-400"
					>
						Pendiente
					</Badge>
				);
		...
		<div className="p-6 space-y-6 max-w-8xl mx-auto">
			...
			<Card className="glass-card overflow-hidden animate-fade-up-delay-1">
				<CardContent className="p-0">
					{isLoading ? (
						<InlineLoader text="Cargando directores..." />
					) : (
						<DataTable
							columns={columns}
							data={directors}
							searchPlaceholder="Buscar directores..."
							showColumnToggle
						/>
					)}
				</CardContent>
			...
			<EntityFormSheet
				title={editingItem ? "Editar Director" : "Nuevo Director"}
				description="Datos de cuenta y asignación académica"
				open={sheetOpen}
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<div className="space-y-2">
						...
						<Input
							...
							type="number"
							value={form.ci}
							onChange={(e) => setForm({ ...form, ci: Number(e.target.value) })}
							required
						/>
					</div>
					<div className="space-y-2">
					...
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setForm({ ...form, paternalSurname: e.target.value })
							}
							required
						/>
					</div>
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setForm({ ...form, maternalSurname: e.target.value })
							}
							required
						/>
					</div>
